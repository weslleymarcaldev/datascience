...
function renderizarChecklistGrupo(grupoNome) {
    ...
    lista.forEach(item => {
        const linha = $(`
            <div class="d-flex align-items-start justify-content-between flex-wrap border-bottom border-secondary-subtle pb-2 mb-2 checklist-row" data-id="${item.id}">
                ...
                <div class="form-check me-2 flex-grow-1">
                    <input class="form-check-input checklist-toggle"
                           ...
                           data-id="${item.id}">
                    <label class="form-check-label text-light checklist-text" data-grupo="${grupoNome}" data-id="${item.id}">
                        ${item.texto}
                    </label>
                </div>

                <div class="btn-group btn-group-sm">
                    <button class="btn btn-outline-info checklist-edit"
                            data-grupo="${grupoNome}"
                            data-id="${item.id}">
                        <i class="bi bi-pencil"></i>
                    </button>
                    <button class="btn btn-outline-danger checklist-del"
                            data-grupo="${grupoNome}"
                            data-id="${item.id}">
...
function renderizarChecklistCompleto() {
    ['programacao', 'matematica', 'datascience', 'ml'].forEach(grupo => {
